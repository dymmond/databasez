...
# Supports both `{!> path !}` and `{! path !}` include styles.
...
def prepare_docs_tree(source_dir: Path, output_dir: Path) -> list[Path]:
    ...
    tmp_output_resolved = tmp_output_dir.resolve()

    for source_file in sorted(source_dir.rglob("*")):
        if source_file.is_dir():
            continue
        resolved_source = source_file.resolve()
        if resolved_source.is_relative_to(output_resolved):
            continue
        if resolved_source.is_relative_to(tmp_output_resolved):
            continue
...
def run_zensical(
    *,
    ...
    try:
        ...
        raise DocsPipelineError(
            "Cannot find `zensical` command. Install docs dependencies first."
        ) from exc
